...
const PDF_LAYOUTS: Record<PdfKey, {
  title: string;
  subtitle: string;
  ...
  'prior-auth': {
    ...
    subtitle: 'PA-2026-04421 · MEDPLAN COMMERCIAL',
    rows: [
      ['patient_name', 'dob'],
  ...
  'referral': {
    ...
    subtitle: 'SR-2026-0872 · BLUE CROSS PREFERRED',
    rows: [
      ['patient_name', 'dob'],
      ['insurance_id'],
...
export function PdfPane({ fields, activeFieldId, pdfKey, onSelectField }: Props) {
  const layout = PDF_LAYOUTS[pdfKey];
  ...
  return (
    <div className="bg-paper text-paper-ink rounded-md p-6 font-body">
      <div className="font-display text-xl mb-1 text-center">{layout.title}</div>
      <div className="font-mono text-[10px] text-paper-ink/60 text-center mb-5">{layout.subtitle}</div>
