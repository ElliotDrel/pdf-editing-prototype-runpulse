...
export interface FillCell {
  ...
  page_number?: number;
  type?: 'text' | 'checkbox';
  checkbox_details?: Array<{ selected: boolean; text: string; center_coord?: number[] }>;
  [k: string]: unknown;
}
...
export async function clearForm(pdf: Uint8Array): Promise<Uint8Array> {
  ...
  const fd = new FormData();
  fd.append('file', new Blob([pdf as BlobPart], { type: 'application/pdf' }), 'sample.pdf');
...
export async function fillForm(
  ...
  if (ctype.includes('application/json')) {
    ...
    const pdfRes = await fetch(body.pdf_url, { headers: { 'x-api-key': apiKey } });
    if (!pdfRes.ok) throw new Error(`pulse pdf_url fetch failed: ${pdfRes.status}`);
    return new Uint8Array(await pdfRes.arrayBuffer());
  }
  return new Uint8Array(await res.arrayBuffer());
}

export function buildInstructions(fields: Array<{ label: string; value: string; type: string }>): string {
